...
## 2024-05-28 - Explicit ARIA states for dynamically disabled buttons
**Learning:** When dynamically disabling a submit button using JavaScript to indicate a loading state, simply setting `disabled = true` is often insufficient for assistive technologies to understand that an ongoing background process is active. Adding `aria-disabled="true"` and `aria-busy="true"` explicitly communicates the inactive state and the active loading process.
**Action:** When adding a visual loading state that disables a submit button, ensure to add `aria-disabled="true"` and `aria-busy="true"` to accurately communicate the state to screen readers.
