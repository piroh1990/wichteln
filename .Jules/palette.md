...
## 2024-04-20 - Missing Loading State on Admin Forms
...
## 2024-04-20 - Decorative Emoji Accessibility
...
## 2024-04-20 - Confirmation Dialog for Non-Destructive Actions
...
## 2024-04-20 - Providing Context for Optional Fields with .form-hint
...
## 2024-04-20 - Screen Reader Context for Loading Forms
**Learning:** When disabling a form submit button to indicate a loading state via JavaScript (e.g., inside `handleFormSubmit`), setting `disabled=true` visually updates the button, but screen readers might not immediately understand that a background process is ongoing.
**Action:** Always explicitly set `aria-disabled="true"` and `aria-busy="true"` attributes on the button element when displaying a loading state to properly inform screen readers of the inactive and busy state.
