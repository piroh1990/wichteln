...
## 2026-04-16 - Adding aria-disabled and aria-busy to Form Loading States\n**Learning:** When a form is submitted and a loading state is triggered via JavaScript, simply disabling the submit button natively isn't fully accessible. Screen readers benefit from explicit `aria-disabled="true"` and `aria-busy="true"` attributes to clearly convey that an operation is ongoing and the interaction is currently unavailable.\n**Action:** When implementing loading states on form submit buttons (e.g., in `handleFormSubmit`), always add `aria-disabled="true"` and `aria-busy="true"`.
