...
## 2026-06-16 - Sibling Focus Selectors for Visually Hidden Form Controls
**Learning:** When hiding native form controls (like radio buttons or checkboxes) to build custom interactive components, applying focus styles directly to the visually hidden input isn't helpful to users. Keyboard navigation visibility is lost.
**Action:** Use a sibling selector (e.g., `input[type="radio"]:focus-visible ~ .visible-wrapper`) to apply explicit `:focus-visible` styles to the surrounding visible wrapper element, ensuring screen reader and keyboard users can see where focus is.
