...
## 2026-04-04 - Dynamic aria-current for Navigation\n**Learning:** Navigation menus need context for screen readers to understand which page is currently active. While visual cues (like an '.active' class) work for sighted users, screen readers rely on 'aria-current="page"'. Dynamically applying this using PHP's 'basename($_SERVER['PHP_SELF'])' is a clean, CSS-free way to improve accessibility.\n**Action:** Always ensure navigation components use 'aria-current="page"' to indicate the active link.
