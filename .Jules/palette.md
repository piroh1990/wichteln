...
## 2026-04-17 - Missing Loading State on Admin Forms
...
## 2026-04-17 - Confirmation Dialog for Non-Destructive Actions
...
## 2026-04-17 - Providing Context for Optional Fields with .form-hint
...
## 2026-04-17 - Accessible Loading States for Submit Buttons
**Learning:** When disabling a button during form submission to show a visual loading state, screen reader users might not receive proper context about the ongoing background process. Adding `aria-disabled="true"` and `aria-busy="true"` explicitly informs assistive technologies that the element is inactive and currently processing an action.
**Action:** When creating custom loading states on interactive elements via JavaScript, always pair visual feedback (like spinners and disabled attributes) with semantic ARIA attributes (`aria-disabled` and `aria-busy`).
