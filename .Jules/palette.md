...
## 2024-05-26 - Screen Reader Context for Form Loading States
**Learning:** When disabling a form submit button to indicate a loading state via JavaScript (e.g., inside `handleFormSubmit`), visually hiding or disabling the button isn't enough for screen readers. Explicitly setting the `aria-disabled="true"` and `aria-busy="true"` attributes properly informs screen readers of the ongoing background process and inactive state.
**Action:** Always add `aria-disabled="true"` and `aria-busy="true"` to buttons when transitioning them into a loading/processing state via JavaScript.
