...
## 2024-05-25 - Missing Loading State on Admin Forms
...
## 2024-05-25 - Decorative Emoji Accessibility
...
## 2024-05-25 - Confirmation Dialog for Non-Destructive Actions
...
## 2024-05-25 - Providing Context for Optional Fields with .form-hint
...
## 2024-05-25 - ARIA Attributes for Loading States
**Learning:** When disabling a form submit button to indicate a loading state via JavaScript (e.g., inside `handleFormSubmit`), setting `disabled=true` is not enough. Screen readers need explicit `aria-disabled="true"` and `aria-busy="true"` attributes to properly announce the inactive state and ongoing background process.
**Action:** When setting up loading states on interactive elements, always explicitly set `aria-disabled="true"` and `aria-busy="true"` to ensure screen readers inform users of the ongoing process and inactive status.
