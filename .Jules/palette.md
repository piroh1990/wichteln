...
## 2026-06-09 - Missing Loading State on Admin Forms
...
## 2026-06-09 - Decorative Emoji Accessibility
...
## 2026-06-09 - Confirmation Dialog for Non-Destructive Actions
...
## 2026-06-09 - Providing Context for Optional Fields with .form-hint
...
## 2026-06-09 - Focus Styles on Custom Interactive Components Over Hidden Native Controls
**Learning:** When hiding native form controls (like radio buttons or checkboxes) using `opacity: 0` to build custom interactive components (like selectable cards), keyboard focus states are lost, preventing keyboard users from knowing which element is focused.
**Action:** Ensure custom interactive components built this way explicitly include a `:focus-visible` rule. Use a sibling selector (e.g., `input[type="radio"]:focus-visible ~ .card-content`) to apply focus styles (like `outline: 3px solid var(--secondary-color)`) to the visible wrapper element.
