...
## 2024-05-25 - aria-busy for Loading States
**Learning:** When disabling a submit button to indicate a loading state via JavaScript (`disabled=true`), screen readers might not properly announce that a background process is ongoing. Setting `aria-busy="true"` explicitly informs assistive technologies of the active state.
**Action:** When adding loading states to interactive elements (especially form submit buttons), always include `aria-busy="true"` alongside any visual indicators or native disabled states.
