...
## 2026-04-27 - Missing Loading State on Admin Forms
...
## 2026-04-27 - Decorative Emoji Accessibility
...
## 2026-04-27 - Confirmation Dialog for Non-Destructive Actions
...
## 2026-04-27 - Providing Context for Optional Fields with .form-hint
...
## 2026-04-27 - Screen Reader Support for Loading Buttons
**Learning:** When disabling a form submit button to indicate a loading state via JavaScript, simply adding the \`disabled\` attribute might silence screen readers or fail to convey that a background process is happening. Adding \`aria-disabled="true"\` and \`aria-busy="true"\` explicitly informs assistive technologies of the inactive state and ongoing work.
**Action:** Always set \`aria-disabled="true"\` and \`aria-busy="true"\` alongside the native \`disabled\` attribute when showing loading states on submit buttons.
